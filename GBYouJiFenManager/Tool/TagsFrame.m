...
- (void)setTagsArray:(NSArray *)tagsArray
{
    ...
    for (NSInteger i=0; i<tagsArray.count; i++) {
        ...
        btnW = [self sizeWithText:model1.DI002 font:TagsTitleFont].width + _tagsMinPadding * 2;
        
        if (i < tagsArray.count-1) {
             POSDIModel *model2=tagsArray[i+1];
            nextWidth = [self sizeWithText:model2.DI002 font:TagsTitleFont].width + _tagsMinPadding * 2;
        }
    ...
    for (NSInteger i=0; i<lastIndexs.count; i++) {
        ...
        for (NSInteger j=0; j<newArr.count; j++) {
            ...
            tagW = [self sizeWithText:model.DI002 font:TagsTitleFont].width + _tagsMinPadding * 2 + averageW;
