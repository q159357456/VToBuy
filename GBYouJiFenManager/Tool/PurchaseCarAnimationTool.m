...
-(void)startAnimationandView:(UIView *)view andRect:(CGRect)rect andFinisnRect:(CGPoint)finishPoint andFatherView:(UIView *)fview andFinishBlock:(animationFinisnBlock)completion
{
    //图层
...
//    AppDelegate *delegate = [UIApplication sharedApplication].delegate;
    ...
    _layer.position = CGPointMake(view.center.x+100, CGRectGetMidY(rect)+64+90); //a 点
        // 路径
    UIBezierPath *path = [UIBezierPath bezierPath];
    [path moveToPoint:CGPointMake(_layer.position.x, _layer.position.y-150)];
    [path addQuadCurveToPoint:finishPoint controlPoint:CGPointMake(ScreenWidth/2-60,rect.origin.y-80)] ;
    //关键帧动画
