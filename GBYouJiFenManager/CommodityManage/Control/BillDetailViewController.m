...
-(void)creatBttom1
{
    ...
    UIView *view=[[UIView alloc]initWithFrame:CGRectMake(0, screen_height-50, screen_width, 1)];
    view.backgroundColor=[UIColor lightGrayColor];
    [self.view addSubview:view];
    button.frame=CGRectMake(0,screen_height-49,screen_width, 49);
...
//        self.personCount=dic1[@"DataSet"][@"Table"][0][@"SB009"];
//        self.memberNo=dic1[@"DataSet"][@"Table"][0][@"SB016"];
    } Faile:^(NSError *error) {
...
-(void)creatBttom
{
    ...
    {
        UIButton *button=[UIButton buttonWithType:UIButtonTypeCustom];
        button.frame=CGRectMake(screen_width/nameArray.count*i,screen_height-59,screen_width/nameArray.count-1, 59);
        [button setImage:[UIImage imageNamed:[NSString stringWithFormat:@"Bill_%ld",i+1]] forState:UIControlStateNormal];
        if (i>0) {
            UIView *lineView=[[UIView alloc]initWithFrame:CGRectMake(screen_width/nameArray.count*i-1,screen_height-50, 1, 40)];
...
//        NSLog(@"－－%@",model.SBP026);
        ...
        cell.tuicaiBlock=^{
            //退
            weakSelf.coverView=[[CoverView alloc]initWithFrame:self.view.frame];
