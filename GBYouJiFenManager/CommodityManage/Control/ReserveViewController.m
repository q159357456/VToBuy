...
//    @"POSSI[A]
    //    POSSTS[A]||POSSI[B]{left (A.company=B.company and A.shopid=B.shopid and A.STS010=B.SI001)}
...
{
    //创建搜索栏
    searchField = [[UITextField alloc] initWithFrame:CGRectMake(15, 77, screen_width-30, 45)];
...
//    [searchBtn addTarget:self action:@selector(SearchBtnClick) forControlEvents:UIControlEventTouchUpInside];
    ...
    UIView*ItemView = [[UIView alloc] initWithFrame:CGRectMake(0, 130, screen_width, 40)];
...
-(void)cretTable
{
    _tableview=[[UITableView alloc]initWithFrame:CGRectMake(0, 170, screen_width, screen_height-170-60) style:UITableViewStyleGrouped];
...
-(void)creatBttom
{
    ...
    for (NSInteger i=0; i<nameArray.count; i++)
    {
        ...
        button.frame=CGRectMake(screen_width/4*i,screen_height-59,screen_width/4-1, 59);
//        [button setImage:[UIImage imageNamed:[NSString stringWithFormat:@"Bill_%ld",i+1]] forState:UIControlStateNormal];
        if (i>0) {
            UIView *lineView=[[UIView alloc]initWithFrame:CGRectMake(screen_width/4*i-1,screen_height-50, 1, 40)];
