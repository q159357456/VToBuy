...
- (void) keyboardWillShow:(NSNotification *)notification {
    ...
    offset = self.table.frame.origin.y+self.table.frame.size.height - (self.height - kbHeight);
    ...
    if(offset > 0) {
        ...
        [UIView animateWithDuration:duration animations:^{
            NSLog(@"%f",-offset);
            self.table.frame = CGRectMake(self.table.x,self.table.y-offset, self.table.width, self.table.height);
        }];
    }
}
///键盘消失事件
- (void) keyboardWillHide:(NSNotification *)notify {
    ...
    //视图下沉恢复原状
    [UIView animateWithDuration:duration animations:^{
        self.table.frame = CGRectMake(self.table.x,self.table.y , self.table.width, self.table.height);
...
-(void)creatUI
{
    ...
    _table.userInteractionEnabled=YES;
     _table.frame=CGRectMake((screen_width-300)/2, self.height/2+110, 300,220);
